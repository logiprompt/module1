(function () {
  ...
  OrderDetailsController.$inject = ['$scope','$http','$state','$stateParams'];
  function OrderDetailsController ($scope, $http, $state) {
   //alert(); 
   $scope.formdata = {};
...
$scope.getMyTab=function(tabid){

$scope.activeTab=tabid;
document.querySelectorAll('.tab-pane.active')[0].classList.remove("active");
document.querySelectorAll('#'+tabid)[0].classList.add("active");

}
$scope.weightsettings='1';
$scope.wsetting=function(val){
  console.log(val);
  $scope.weightsettings=val;
}
