(function () {
  ...
  angular
    .module('core')
    .controller('EditEmailshipmentcommentsController', EditEmailshipmentcommentsController);



    EditEmailshipmentcommentsController.$inject = ['$scope','$http','$state','$stateParams', 'Upload','shipmentcommentsService'];

  function EditEmailshipmentcommentsController ($scope, $http, $state, $stateParams, Upload, shipmentcommentsService) {
 ...
 $scope.currentLan=localStorage.getItem('currentLang').toString();
  $scope.defaultLang=localStorage.getItem('defaultLang').toString();
    ...
    $scope.getShipmentCommentsById = function (userId) {
      console.log(0);
      $scope.shipmentcommentsService.getShipmentCommentsById(userId).then(function (result) {
        console.log(userId);
         console.log(result);
    ...
    $scope.getShipmentCommentsById($stateParams.id);
...
///////////////////////////////////////////////////////////////
    ...
    /*
     *
     *  Function : update ShipmentComments
     * Description : Update ShipmentComments details
     * Owner : anju
     * 
     */

    $scope.updateShipmentComments = function () {
      ...
      if ($scope.formdata.$valid && $scope.status != 0) {
        ...
        $scope.shipmentcommentsService.updateShipmentComments($stateParams.id, data).then(function (result) {
          console.log(result);
          if (result.statusText = "OK") {
            swal("Sccess!", "Successfully updated ", "success");
            $state.go('emailshipmentcomments');
          }
        });
      }
    }
