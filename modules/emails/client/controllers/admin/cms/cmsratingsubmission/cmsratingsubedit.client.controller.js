(function () {
  ...
  angular
    .module('emails')
    .controller('CmsRatingSubEdit', CmsRatingSubEdit);



  CmsRatingSubEdit.$inject = ['$scope', '$http', '$state', '$stateParams', 'Upload', 'cmsRatingSubService'];

  function CmsRatingSubEdit($scope, $http, $state, $stateParams, Upload, cmsRatingSubService) {
    ...
    $scope.delUserForgot = function (userId) {
      ...
      swal({
        ...
        if (result) {
          $scope.cmsRatingSubService.delUserForgot(userId).then(function (result) {
   ...
   */
    ...
    //console.log( $scope.prdRatingSubService);
    $scope.currentLan = localStorage.getItem('currentLang').toString();
    $scope.defaultLang = localStorage.getItem('defaultLang').toString();

    $scope.getCmsRatingSubById = function (userId) {
      //console.log(0);
      $scope.cmsRatingSubService.getCmsRatingSubById(userId).then(function (result) {
        //console.log(result);
    ...
    $scope.getCmsRatingSubById($stateParams.id);
    ...
    $scope.updatecmsRatingSub = function () {
      ...
      if ($scope.formdata.$valid && $scope.status != 0) {
        ...
        //console.log(data);
        $scope.cmsRatingSubService.updatecmsRatingSub($stateParams.id, data).then(function (result) {
          if (result.statusText = "OK") {
            swal("Sccess!", "Successfully updated", "success");
            $state.go('emailcmsratingsubmission');
          }
        });
      }
    }
