(function () {
  ...
  angular
    .module('core')
    .controller('OrdercancelationlistController', OrdercancelationlistController);



    OrdercancelationlistController.$inject = ['$scope','$http','$state','$stateParams', 'Upload','cancelationService'];

  function OrdercancelationlistController ($scope, $http, $state, $stateParams, Upload,cancelationService) {
...
//$scope.currentLang= localStorage.getItem('currentLang').toString();
...
//console.log(userId);
  swal({
    ...
    if(result){
    $scope.cancelationService.delCheckedCancelation(userId).then(function(result){
    if(result.statusText = "OK"){
    swal('Deleted!',
