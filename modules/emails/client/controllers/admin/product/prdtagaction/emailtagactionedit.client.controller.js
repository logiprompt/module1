(function () {
  ...
  angular
    .module('emails')
    .controller('EmailpdttagactionEdit', EmailpdttagactionEdit);



  EmailpdttagactionEdit.$inject = ['$scope', '$http', '$state', '$stateParams', 'Upload', 'prdTagActionService'];

  function EmailpdttagactionEdit($scope, $http, $state, $stateParams, Upload, prdTagActionService) {
    ...
    $scope.delUserForgot = function (userId) {
      ...
      swal({
        ...
        if (result) {
          $scope.prdTagActionService.delUserForgot(userId).then(function (result) {
   ...
   */
    ...
    //console.log( $scope.prdTagActionService);
    $scope.currentLan = localStorage.getItem('currentLang').toString();
    $scope.defaultLang = localStorage.getItem('defaultLang').toString();

    $scope.getprdTagActionById = function (userId) {
      //console.log(0);
      $scope.prdTagActionService.getprdTagActionById(userId).then(function (result) {
        //console.log(result);
    ...
    $scope.getprdTagActionById($stateParams.id);
    ...
    $scope.updateprdTagAction = function () {
      ...
      if ($scope.formdata.$valid && $scope.status != 0) {
        ...
        //console.log(data);
        $scope.prdTagActionService.updateprdTagAction($stateParams.id, data).then(function (result) {
          if (result.statusText = "OK") {
            swal("Sccess!", "Successfully updated User", "success");
            $state.go('emailproducttagaction');
            // $state.reload();
          }
        });
      }
