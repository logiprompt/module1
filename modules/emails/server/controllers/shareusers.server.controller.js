...
var path = require('path'),
    ...
    ShareWithUsers = mongoose.model('Sys_ShareWithUsers'),
...
exports.create = function(req, res) {
    ...
    ShareWithUsers.create(userDetails,function(err) {
        if (err) {
...
exports.list = function(request, response) {
    ...
    ShareWithUsers.find().exec(function(error, items) {
...
{ 
  ...
  ShareWithUsers.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
   ...
   ShareWithUsers.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            console.log(error);
	...
	exports.delete = function(request, response) {
        var userId = request.query.userId;
         console.log(userId);
         ShareWithUsers.findById(userId).exec(function (error, item) {
	...
	exports.update = function (request, response) {
        ...
        ShareWithUsers.findById(userId).lean().exec(function (error, data) {
            ...
            if (error) {
                ...
                if (reqBody.isDefaultLang) {
                 ...
                 ShareWithUsers.update({'_id':userId}, 
      ...
      **/
      exports.delCheckedOrderHold = function(request, response) {
          ...
          console.log(arr);
          ShareWithUsers.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
