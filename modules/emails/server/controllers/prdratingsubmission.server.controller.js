...
var path = require('path'),
    ...
    prdRatingSubmission = mongoose.model('Sys_PrdRatingSubmission'),
...
exports.create = function(req, res)
{
  ...
  prdRatingSubmission.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    prdRatingSubmission.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  prdRatingSubmission.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    prdRatingSubmission.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
         prdRatingSubmission.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        prdRatingSubmission.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                prdRatingSubmission.update({'_id':userId}, 
      ...
      *
      **/
      exports.delCheckedprdRatingSub = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          prdRatingSubmission.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
