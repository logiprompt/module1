...
var path = require('path'),
    ...
    prdTagAction = mongoose.model('Sys_PrdTagAction'),
...
exports.create = function(req, res)
{
  var userDetails = req.body; 
   prdTagAction.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
    console.log(919191919191919);
     prdTagAction.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
   prdTagAction.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    // console.log(90909090909090)
     prdTagAction.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
          prdTagAction.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
      ...
      **/
      ...
      exports.delCheckedprdTagAction = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
           prdTagAction.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
