...
var path = require('path'),
    ...
    cmsRatingSubmission = mongoose.model('Sys_CmsRatingSub'),
...
exports.create = function(req, res)
{
  ...
  cmsRatingSubmission.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    cmsRatingSubmission.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  cmsRatingSubmission.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    cmsRatingSubmission.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
         cmsRatingSubmission.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        cmsRatingSubmission.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                cmsRatingSubmission.update({'_id':userId}, 
      ...
      *
      **/
      exports.delCheckedcmsRatingSub = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          cmsRatingSubmission.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
