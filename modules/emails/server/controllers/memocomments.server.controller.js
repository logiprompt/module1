...
var path = require('path'),
    ...
    MemoComments = mongoose.model('Sys_MemoComments'),
...
exports.create = function(req, res)
{
  ...
  MemoComments.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    MemoComments.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  MemoComments.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    MemoComments.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
         MemoComments.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        MemoComments.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                MemoComments.update({'_id':userId}, 
      ...
      **/
      ...
      exports.delCheckedMemoComments = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          MemoComments.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
