...
var path = require('path'),
    ...
    prdTagSubmission = mongoose.model('Sys_PrdTagSubmission'),
...
exports.create = function(req, res)
{
  ...
  prdTagSubmission.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    prdTagSubmission.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  prdTagSubmission.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    prdTagSubmission.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
         prdTagSubmission.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        prdTagSubmission.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                prdTagSubmission.update({'_id':userId}, 
      ...
      **/
      ...
      exports.delCheckedprdtagSub = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          prdTagSubmission.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
