...
var path = require('path'),
    ...
    Orderunhold = mongoose.model('Sys_OrderUnhold'),
...
exports.create = function(req, res) {
    ...
    Orderunhold.create(userDetails,function(err) {
        if (err) {
...
exports.list = function(request, response) {
    ...
    Orderunhold.find().exec(function(error, items) {
...
{ 
  ...
  Orderunhold.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
   ...
   // console.log(90909090909090)
    Orderunhold.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            console.log(error);
	...
	exports.delete = function(request, response) {
        var userId = request.query.userId;
         console.log(userId);
         Orderunhold.findById(userId).exec(function (error, item) {
	...
	exports.update= function(request, response){
		...
		Orderunhold.findById(userId).exec(function (error, data) {
			  if (error) {
			        response.status(500).send(error);
			        return;
			      }
      ...
      *
      **/
      exports.delCheckedOrderUnhold = function(request, response) {
      
          var arr = request.query.userId;
          console.log(arr);
          Orderunhold.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
