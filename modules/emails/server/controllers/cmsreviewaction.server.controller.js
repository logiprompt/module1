...
var path = require('path'),
    ...
    cmsReviewAction = mongoose.model('Sys_CmsReviewAction'),
...
exports.create = function(req, res)
{
  ...
  cmsReviewAction.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    cmsReviewAction.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  cmsReviewAction.findById(request.query.userId)
    .lean()
...
{
    cmsReviewAction.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
         cmsReviewAction.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        cmsReviewAction.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                cmsReviewAction.update({'_id':userId}, 
      ...
      **/
      ...
      exports.delCheckedcmsReviewAction = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          cmsReviewAction.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
