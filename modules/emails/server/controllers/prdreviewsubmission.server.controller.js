...
var path = require('path'),
    ...
    prdReviewSubmission = mongoose.model('Sys_PrdReviewSubmission'),
...
exports.create = function(req, res)
{
  ...
  prdReviewSubmission.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    prdReviewSubmission.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  prdReviewSubmission.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    prdReviewSubmission.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
         prdReviewSubmission.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        prdReviewSubmission.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                prdReviewSubmission.update({'_id':userId}, 
      ...
      **/
      ...
      exports.delCheckedprdReviewSub = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          prdReviewSubmission.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
