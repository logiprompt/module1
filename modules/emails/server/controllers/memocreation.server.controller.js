...
var path = require('path'),
    ...
    MemoCreation = mongoose.model('Sys_MemoCreation'),
...
exports.create = function(req, res)
{
  var userDetails = req.body; 
    MemoCreation.create(userDetails,function(err) {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    MemoCreation.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  MemoCreation.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    MemoCreation.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
         MemoCreation.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        MemoCreation.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                MemoCreation.update({'_id':userId}, 
      ...
      **/
      ...
      exports.delCheckedMemoCreation = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          MemoCreation.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
