...
var path = require('path'),
    ...
    Nostock = mongoose.model('Sys_NoStock'),
...
exports.create = function(req, res)
{
  var userDetails = req.body; 
    Nostock.create(userDetails,function(err) {
        if (err)
        {
...
exports.list = function(request, response) 
{
    ...
    Nostock.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
  Nostock.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    Nostock.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        var userId = request.query.userId;
         console.log(userId);
         Nostock.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        Nostock.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
      ...
      **/
      ...
      exports.delCheckedNoStock = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
          Nostock.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
