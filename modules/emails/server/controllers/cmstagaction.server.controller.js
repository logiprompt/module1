...
var path = require('path'),
    ...
    cmsTagAction = mongoose.model('Sys_CmsTagAction'),
...
exports.create = function(req, res)
{
  var userDetails = req.body; 
   cmsTagAction.create(userDetails,function(err)
  {
        if (err)
        {
...
exports.list = function(request, response) 
{
     ...
     cmsTagAction.find().exec(function(error, items) 
    {
        if (error) 
        { 
...
{ 
   cmsTagAction.findById(request.query.userId)
    .lean()
...
exports.userByIDs = function(request, response) 
{
    ...
    // console.log(90909090909090)
     cmsTagAction.findById(request.params.userId)
    .lean()
    .exec(function(error, items) {
        if (error) {
            //console.log(error);
	...
	exports.delete = function(request, response) {
        ...
        // console.log(userId);
          cmsTagAction.findById(userId).exec(function (error, item) {
    ...
    exports.update= function(request, response)
    {
        ...
        //console.log(userId);
         ...
         cmsTagAction.findById(userId).exec(function (error, data) 
        {
              if (error) 
              {
			     response.status(500).send(error);
              ...
              if (data) 
              {
                ...
                else 
                {
                 ...
                 cmsTagAction.update({'_id':userId}, 
      ...
      **/
      ...
      exports.delCheckedcmsTagAction = function(request, response) 
      {      
          var arr = request.query.userId;
         // console.log(arr);
           cmsTagAction.deleteMany({_id:{'$in':arr}}).exec(function (err, data) {
                               if (err) throw err;
                               response.json({
                                              status: 1,
                                  });	
